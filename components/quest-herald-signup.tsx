...
export function QuestHeraldSignup({
  ...
  if (shouldHideQuestHeraldSignup(status, { forceVisible, succeeded })) {
    return null
  }
  ...
  return (
    <div className={questHeraldRootClassName(compact, className)}>
      <div className={questHeraldCardClassName(compact)}>
        <QuestHeraldCardHeader
          heading={heading}
          compact={compact}
          onDismiss={handleDismiss}
        />
        {succeeded ? (
          <QuestHeraldSuccessMessage />
        ) : (
          <QuestHeraldEmailForm
            compact={compact}
            email={email}
            isPending={isPending}
        ...
        <QuestHeraldErrorMessage error={error} />
      </div>
    </div>
  )
}

function questHeraldRootClassName(compact: boolean, className?: string) {
  return cn(
...
function questHeraldCardClassName(compact: boolean) {
  return cn(
...
function QuestHeraldCardHeader({
  heading,
  compact,
  onDismiss,
}: {
...
function QuestHeraldSuccessMessage() {
  return (
...
function QuestHeraldErrorMessage({ error }: { error: string | null }) {
  if (!error) {
    return null
  }
