...
Generated by the improve skill on 2026-07-06 at commit `ba0778c`; extended and reconciled on 2026-07-11 at commit `16f120d`; reconciled again on 2026-07-11 at commit `ab0a47e`; reconciled on 2026-07-13 at commit `f145ca21`.
...
Executor model preference for this backlog: Grok 4.5 only.
...
| 007 | Decompose the 1383-line map-shell.tsx | P3 | L | 003 DONE, 006 DONE | DONE (verified on `advisor/007-decompose-map-shell` @ `a1bdd535`; map-shell 366 lines; browser Step 4 SKIPPED) |
...
- 2026-07-13 (`/improve execute 007`, Grok 4.5): APPROVED. Worktree `/Users/taishikato/.cursor/worktrees/aistartupquest/007-decompose-map-shell`, branch `advisor/007-decompose-map-shell` (stacked on 006), commits `fb9d6cd0` → `5adbadea` → `a1bdd535`. Extracted `lib/map-paint.ts`, `components/map-markers/sprites.ts`, `components/map-markers/use-map-markers.ts`; `map-shell.tsx` 366 lines; single `setPaintPropertyIfLayerExists`. Reviewer re-ran typecheck/test(58)/lint/build; scope clean. Documented deviations: marker unmount cleanup owned by the hook; `MAP_PITCH`/`MAP_BEARING` duplicated with sync comment. Manual browser Step 4 SKIPPED. Not merged to main.
...
- 2026-07-13 (`/improve reconcile` at `f145ca21`, Grok 4.5 only):
  - **DONE spot-checks (cheap)**: Next `16.2.10` (001); CI present (003); Turnstile on company+meetup actions (002); `next-themes` absent (004); `map-libre` gone (011); `lib/cursor-community-events.ts` present (015); sitemap/robots/structured-data present (017); `lib/home-map-url.ts` present (019); fetch path https-filters URLs (014). `pnpm typecheck` / `test` (58) / `lint --max-warnings 0` all green.
  - **TODO**: 016 finding still at `home-events-map.tsx:478-502` — refreshed Current state for post-015/019 code; 006/007 unchanged (`map-shell.tsx` zero drift since `ba0778c`, still 1383 lines).
  - **Ordering note**: 019 landed before 016; 016 must not regress URL sync.
  - **Housekeeping**: stale worktree `~/.cursor/worktrees/aistartupquest/008-refresh-docs` on deleted remote `advisor/008-refresh-docs` @ `034325ff` — safe to `git worktree remove` when convenient. No IN PROGRESS / BLOCKED plans.
  - **Executable now**: 006 → 007 (016 DONE).
  - Residual `pnpm audit --prod` (38 vulns, 6 high) is mostly transitive (e.g. hono via shadcn MCP path, babel via next); not a new Next floor after 001.
...
- 006 and 007 require 003 because no automated tests cover the map components; the CI gate (lint zero-warnings, typecheck, test) is the only regression net for those refactors. 003, 006, and 007 are DONE on advisor branches (007 stacked on 006); merge `advisor/007-decompose-map-shell` to land both.
- 007 required 006 to land first so the diffing logic moved into `useMapMarkers` wholesale.
...
- 015, 016, and 019 are DONE (016 merged PR #31 → `fc7fd5c0`). Next map work is 006 → 007.
- 018 designs against 017's `buildPageMetadata` helper and `app/sitemap.ts` when they exist; both 017 and 018 are DONE.
- 017 and plan 008 both touch product naming ("AI Startup Quest"); both DONE — keep wording consistent in future SEO work.
...
- 2026-07-13 reconcile at `f145ca21`: no new audit; verified DONE cheap criteria; refreshed 016 for post-015/019 drift; confirmed 006/007 still match `ba0778c` excerpts.
